...
if (isset($_POST['register'])) {
	...
	if (empty($rol)) {
		$input_error['rol'] = "El rol es un campo requerido";
	}
	...
	if (count($input_error) == 0) {
		...
		if (mysqli_num_rows($check_email) == 0) {
			$check_username = mysqli_query($conexion, "SELECT * FROM `users` WHERE `username`='$username';");
			if (mysqli_num_rows($check_username) == 0) {
				if (strlen($username) > 7) {
					if (strlen($password) > 7) {
						$password = sha1(md5($password));
						$query = "INSERT INTO `users`(`name`, `email`, `username`, `password`, `photo`, `status`,`rol` ) VALUES ('$name', '$email', '$username', '$password','$photo_name','inactivo', '$rol');";
...
<body>
	...
	<br><br><br>
	<p style="text-align: center;">Si tienes una cuenta de acceso administrativo, puedes <a href="login.php">Ingresar Aquí</a></p>
	</div>
